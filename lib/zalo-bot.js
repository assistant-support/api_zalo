...
/**
 * Xử lý yêu cầu đăng nhập bằng mã QR với đầy đủ log chi tiết.
...
export async function handleQrLogin(io, socket) {
    ...
    try {
                    ...
                    const checkInterval = setInterval(() => {
                        try {
                            ...
                            if (stats.size > 0) {
                                ...
                                console.log(`[QR STAGE] ✅ SUCCESS: Sending Base64 QR data (length: ${base64String.length}) to client.`);
                            ...
                            // Bỏ qua lỗi và thử lại trong lần lặp tiếp theo
                        }
                    }, 100);
                }
            });
        ...
        api.listener.on('message', (message) => {
            ...
            const isDirectMessage = messageType === ThreadType.User;
            const isFromTarget = String(senderId) === String(targetUserId);

            console.log(`[FILTERING] Comparing senderId (${senderId}) with targetUserId (${targetUserId}). Match: ${isFromTarget}`);
            console.log(`[FILTERING] Message type is: ${messageType} (Is Direct Message: ${isDirectMessage})`);

            if (isDirectMessage && isFromTarget) {
                ...
                if (typeof message.data.content === 'string') {
                    ...
                    displayContent = '[Bạn đã nhận một sticker]';
                } else {
                    displayContent = '[Tin nhắn không phải dạng văn bản]';
                }
        ...
        // Lắng nghe sự kiện logout hoặc lỗi để tự động kết nối lại
        api.listener.on('logout', (reason) => {
            console.error(`[LISTENER] ❌ LOGOUT EVENT DETECTED. Reason: ${reason}`);
            globalThis.zaloApi = null; // Reset trạng thái
            globalThis.receivedMessages = []; // Xóa tin nhắn cũ
            // Thông báo cho tất cả client rằng phiên đã hết hạn
            io.emit('login_status', { isLoggedIn: false, reason: 'Session expired or logged out.' });
        });
        ...
        api.listener.on('error', (err) => {
            ...
            io.emit('login_status', { isLoggedIn: false, reason: 'Connection error.' });
        });
